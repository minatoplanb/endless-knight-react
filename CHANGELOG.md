...
- **掉落通知系統（Loot Notification System）**
  - 稀有素材（💎）和 Boss 素材（👑）掉落時顯示浮動通知
  - 通知顯示在戰鬥畫面右下角
  - 動畫效果：彈入 → 停留 1.5 秒 → 淡出
  - 避免普通素材通知刷屏

- **技能長按說明（Skill Tooltip）**
  - 長按技能按鈕顯示技能詳細資訊
  - 顯示技能名稱、等級、效果、冷卻時間
  - 顯示每級效果增加量

- **消耗品長按說明（Consumable Tooltip）**
  - 長按消耗品按鈕顯示詳細資訊
  - 顯示名稱、類型（食物/藥水）、效果描述
  - 回復類顯示 HP 數值
  - 增益類顯示效果百分比和持續時間
...
- **裝備篩選/排序（Equipment Filtering & Sorting）**
  - 按類型篩選：全部/武器/頭盔/盔甲/盾牌/戒指/項鏈
  - 按等級、稀有度、強化等級排序
  - 可收合的篩選面板

- **連殺顯示（Kill Streak Display）**
  - 連殺達到 5 時顯示 🔥 徽章
  - 顯示在戰鬥畫面右上角

- **區域進度條（Area Progress Bar）**
  - 在敵人進度條下方新增區域進度條
  - 顯示當前關卡/總關卡數（如：Stage 15/100）
  - 金色填充條顯示區域完成進度
  - 支援 i18n（中/英文）

- **區域長按說明（Area Tooltip）**
  - 長按區域按鈕顯示詳細資訊
  - 顯示區域名稱、進度、推薦等級、敵人類型數
  - 區域選擇器支援 i18n
